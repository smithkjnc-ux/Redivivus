# Redivivus v1.0 — Design Specification
### 🔒 Blueprint Locked — Built by PapaJoe
---

## What Redivivus Is

Redivivus is an AI-agnostic VS Code extension + Universal Project Protocol. It loads your structure/annotation standards into any project, routes to swappable AI backends (Claude/Gemini/Llama/etc), and enforces consistency regardless of which AI is doing the work. The AI is the engine. Redivivus is the frame everything bolts to.

**Built around one question:** What actually works best for the person building?

## What Redivivus Is Not
...
1. **AI-Agnostic** — Any AI slots in. Claude, Gemini, DeepSeek, Llama, whatever comes next. Redivivus doesn't care. Your rules stay constant.
2. **User's Rules First** — The protocol enforces YOUR standards, not the AI's preferences. The AI provides horsepower, Redivivus controls the transmission.
3. **No Lock-In** — Remove Redivivus and your code still works. The annotations are comments. The WORK_LOG is markdown. Nothing proprietary.
...
5. **Lazy User Principle** — Assume the user will give minimal information. Redivivus adapts and asks smart follow-ups instead of demanding a perfect spec upfront.
...
7. **Scope Guardian** — Redivivus detects feature creep. If the AI starts building something that wasn't in the blueprint, it flags it before writing code.
...
10. **Guardian First** — Redivivus prioritizes architectural health and security over AI speed. If an AI suggests a "quick fix" that breaks modularity or safety, Redivivus blocks and mentors the user on the correct path.
11. **Visual-to-Code Symmetry** — The user’s visual experience and the underlying code must be treated as a single entity. If a user can see it, Redivivus must be able to find and modify the code that generates it.

---

## Five W Blueprint Interview

Before any code is written, Redivivus asks five questions. Each one gets a "here's why I'm asking" preamble so the user understands the purpose.
...
What problem does it solve that isn't already solved? This is the gut check. If the answer is weak, Redivivus flags it before anyone writes code.
...
- 🔶 **Assumed** — Redivivus inferred this from context, needs verification
- ❓ **Unknown** — Critical gap, must be resolved before building

### Blueprint Health Score
Before building starts, Redivivus reports:
```
...
The Universal Project Protocol. Standards for file structure, naming, documentation format. Lives in a `.redivivus/` directory in the project root. Portable across projects.

**Contract:** Any project with a `.redivivus/` folder follows the same rules regardless of AI or editor.
...
**Contract:** These are plain comments. They work in any language, any editor, any AI. Removing Redivivus doesn't break anything.
...
AI backend selection. The user picks which AI handles the work. Redivivus routes the request, injects the protocol context, and returns the result.
...
Community-contributed blueprints, logic blocks, and solutions. Quality-gated: only Redivivus-verified code can be uploaded. The extension is the gatekeeper.

**Contract:** Everything in the Vault has a paper trail — the Blueprint, the WORK_LOG, the exit interview. Bad code can't get a Redivivus stamp.
...
### Exit Interview
End of every session, Redivivus prompts:
...
Redivivus monitors for feature creep:
...
- User decides — Redivivus just makes sure it's intentional
...
The session history, decision log, and project spec. Lives in `.redivivus/` directory.
...
Redivivus VS Code extension with Protocol, Annotation, and Control layers. Blueprint Interview. WORK_LOG. Measure Twice. Works standalone with no AI routing.
...
**"Do AI Dream?"** podcast — the first project built entirely under Redivivus protocol to validate the system.

---

*🔒 Blueprint Locked — Redivivus v1.0 — Built by PapaJoe*
