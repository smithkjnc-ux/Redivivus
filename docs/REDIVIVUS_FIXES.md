...
- **`supervisorOrchestrator.ts`**: Refactored the `buildPlanPrompt` to make the Supervisor act as a "True Architect" by enforcing a strict JSON contract. Replaced the generic `spec` string with explicit schema keys: `filesToCreate`, `dependencies`, and `exactInstructions`. This forces the Supervisor to define the global file tree upfront, preventing filename hallucinations (like `file1.css`).
- **`supervisorOrchestrator.ts` & `chatPanelBuildOrchestrated.ts`**: Updated the Guardian's `reviewOutput` loop to explicitly check the generated code against the strict contract (`planContext`) rather than just the generic user task, ensuring the Worker AI did not deviate from the Supervisor's blueprint.
- **`supervisorOrchestrator.ts` & `chatPanelBuildOrchestrated.ts`**: Added full-payload pipeline logging using `redivivusLogger`. The orchestrator now logs the complete `PlanStep[]` JSON array, the exact Worker prompt/response, and the exact Guardian review prompt/response to `.redivivus/logs/session-*.log`. This creates a permanent, searchable audit trail of the AI's internal dialogue and contracts for debugging hallucinations.
- **`chatPanelBuildRunner.ts`**: Added logic to automatically pop open the `.redivivus/logs/session-*.log` file in a split editor when building a new project (which is built in the background). This allows users to watch the AI's diagnostics and planning stream in real-time, even before the project folder is formally mounted into the VS Code workspace.
- **`redivivus-backend` (Rule 9)**: Updated `redivivusWorkerRules.ts` on the backend so Rule 9 does *not* force single-file HTML unless explicitly requested or for trivial scripts. Additionally, explicitly enforced `.js` over `.ts` for vanilla web projects.
...
## May 30, 2026 — Session 11DO (Fix: launcher opening two windows — --new-window → --reuse-window)

**Root cause:** `install.sh` (generated by `postcompile-deploy.js`) set `--new-window` in the Exec line, forcing a new IDE window every click regardless of whether one was already open. `release.sh` developer desktop also lacked `--reuse-window`.

**Changes:**

| File | What Changed | Why | Risk |
|---|---|---|---|
| `scripts/postcompile-deploy.js` | `--new-window` → `--reuse-window` in generated install.sh Exec line | `--new-window` always forces a second window. `--reuse-window` focuses existing window or opens new if none. | None — behavior change only for subsequent installs. |
| `scripts/release.sh` | Added `--reuse-window` to developer desktop Exec line | Developer desktop was missing the flag, same behavior. | None. |
| `.local/share/applications/redivivus.desktop` | `--reuse-window` added to Exec immediately. | Immediate fix for current install. | None. |

---

## May 30, 2026 — Session 11DN (Feature: Health button green/yellow/red status indicator)

**Changes:**

| File | What Changed | Why | Risk |
|---|---|---|---|
| `chatPanelHealthCheck.ts` | Added `getHealthStatus(d)` — returns `'red'` if API unreachable, build endpoint down, or 0 keys; `'yellow'` if latency >= 800ms, not signed in, < 2 keys, or fail rate >= 30%; `'green'` otherwise. | Needed to derive a single status level from health data for button coloring. | None — pure function, no side effects. |
| `chatPanelMsgRunCommand.ts` | After health check: calls `getHealthStatus()`, saves result to `globalState('redivivus.healthStatus')`, calls `deps.refresh()` to re-render header. | Persists status so button color survives panel refreshes. | Low — `globalState.update` is safe; `deps.refresh()` is already called elsewhere. |
| `chatPanelHtml.ts` | Added `healthStatus?: 'green' \| 'yellow' \| 'red'` to `ChatHeaderInfo`; Health button now applies border/text color from `healthStatus` and switches dot from ring (no status) to filled (status set). | User-visible: button shows ● green/yellow/red after first health check. | None. |
| `chatPanelHeader.ts` | Returns `healthStatus` from `extensionContext.globalState` in header builder. | Feeds persisted status into every header refresh. | None — optional field, undefined if never checked. |
