...
- **`supervisorOrchestrator.ts`**: Refactored the `buildPlanPrompt` to make the Supervisor act as a "True Architect". Removed hardcoded constraints (like forcing 3 steps for games) and replaced the specific 3-step JSON example with an abstract schema. Added instructions for Environment Assessment, Infrastructure First, Logical Decomposition, and Interface Contracts. This ensures the Supervisor dynamically decides the architecture (e.g. Vanilla vs React) based on the workspace environment rather than blindly templating files. Added an explicit `DO NOT use placeholders or leave functions empty` instruction in `executeStep` to force the Worker AI to generate full code instead of tiny stubs.
