...
- **`supervisorOrchestrator.ts`**: Expanded the `buildPlanPrompt` to explicitly require 3-step builds (HTML, CSS, JS) for games and web apps, overriding the single-step bias. Expanded the JSON example to 3 items. Added an explicit `DO NOT use placeholders or leave functions empty` instruction in `executeStep` to force the Worker AI to generate full code instead of tiny stubs.
