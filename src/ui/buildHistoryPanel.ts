...
//   📍 Save Points — user-created git checkpoints (existing SavePointService)
//   🏗️ Build History — automatic log of every successful CHASSIS build
// Opened by the 💾 Save Point toolbar button.
...
export function showBuildHistoryPanel(context: vscode.ExtensionContext): void {
  ...
  _panel.webview.onDidReceiveMessage(async (msg) => {
    if (msg.type === 'get-data') {
      ...
      if (result.success) {
        vscode.window.showInformationMessage(`↩ Restored to save point`);
        _sendData(root);
      } else {
        vscode.window.showErrorMessage(result.message);
      }
      ...
      const result = await svc.create(desc);
      if (result.success) {
        vscode.window.showInformationMessage(`💾 Promoted to save point: ${desc.slice(0, 40)}`);
        _sendData(root);
      } else {
        vscode.window.showErrorMessage(result.message);
      }
  ...
  // Data is fetched by the webview on load via get-data message
}
...
function _sendData(root: string): void {
  ...
  // Merge orphan snapshots (no matching history entry) as "Unknown build" entries
...
<body>
<div class="tabs">
  <button class="tab active" id="tab-sp" onclick="switchTab('sp')">📍 Save Points</button>
  <button class="tab" id="tab-bh" onclick="switchTab('bh')">🏗️ Build History</button>
</div>
<div class="pane active" id="pane-sp">
  <div class="create-row">
    <input id="sp-input" type="text" placeholder="Save point description (optional)">
    <button class="btn btn-primary" onclick="createSavePoint()">💾 Create Save Point</button>
  </div>
    ...
    el.innerHTML = sps.map(sp => {
      ...
      return \`<div class="sp-entry">
        ...
        <div style="margin-top:8px;display:flex;gap:6px;">
          <button class="btn btn-danger" id="restore-\${escHtml(sp.hash)}" onclick="restoreSavePoint('\${escHtml(sp.hash)}', this)">↩ Restore</button>
        </div>
      </div>\`;
    }).join('');
  }
  ...
  function renderHistory(history) {
    ...
    el.innerHTML = history.map(e => {
      ...
      return \`<div class="entry\${e.undone?' undone':''}" id="entry-\${escHtml(e.id)}">
        ...
        <div class="btn-row">
          <button class="btn btn-danger" id="undo-\${escHtml(e.id)}" \${disabledUndo} onclick="undoBuild('\${escHtml(e.id)}', this)">↩ Undo</button>
          <button class="btn" onclick="promoteSavePoint('\${escHtml(e.id)}')">📍 Save as Checkpoint</button>
        </div>
      </div>\`;
    }).join('');
  }
