  ...
  return `
    document.addEventListener('click', (e) => {
      ...
      if (target.classList&&target.classList.contains('create-file-btn')) {
        ...
        // [FIX 1] AUTO-POPULATE FILENAME
        var rawCode = ''; try { rawCode = atob(c64); } catch(err){}
        var lines = rawCode.split('\\n');
        var suggestedName = 'file.' + ext;
        if (lines.length > 0) {
          ...
          else if (firstLine.includes('[SCOPE]')) {
            var scopeMatch = firstLine.match(/\\[SCOPE\\]\\s*([\\w\\.\\-\\/]+\\.[a-z0-9]+)/i);
            if (scopeMatch) suggestedName = scopeMatch[1];
          }
        }
