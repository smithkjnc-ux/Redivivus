...
import { handleInterviewMessage } from '../../views/blueprintInterviewPanel';
import type { BuildContext } from '../../../core/build/chatPanelBuild';
import { classifyIntent, isBuildRequest, handleBuildRequest, handleEditRequest, BuildRequestDeps } from '../../../core/ai/chatPanelIntent';
import { handleChatMessage } from '../../../core/routing/chatPanelMessages';
import { buildHeaderInfo } from './chatPanelHeader';
import { SetupProgressService, SetupProgress } from '../../../services/project/setupProgressService';
...
export class ChatPanel {
  ...
  private _logBuildError(task: string, prompt: string, error: string, promptTokens = 0): void {
    const { panelLogBuildError } = require('../../../core/build/chatPanelBuildUtils.js');
    return panelLogBuildError(this, task, prompt, error, promptTokens);
  }

  private _buildRequestDeps(): import('../../../core/ai/chatPanelIntent').BuildRequestDeps {
    const { panelBuildRequestDeps } = require('../../../core/build/chatPanelBuildUtils.js');
    return panelBuildRequestDeps(this);
  }

  private async _classifyIntent(text: string) {
    const { panelClassifyIntent } = require('../../../core/build/chatPanelBuildUtils.js');
    return panelClassifyIntent(this, text);
  }

  private async _isBuildRequest(text: string) {
    const { panelIsBuildRequest } = require('../../../core/build/chatPanelBuildUtils.js');
    return panelIsBuildRequest(this, text);
  }

  private async _handleBuildRequest(task: string, skipComplex = false, isFixRequest = false) {
    const { panelHandleBuildRequest } = require('../../../core/build/chatPanelBuildUtils.js');
  ...
  private async _handleVaultOnlyBuild(task: string): Promise<void> {
    const { panelVaultOnlyBuild } = require('../../../core/build/chatPanelBuildUtils.js');
    return panelVaultOnlyBuild(this, task);
  }

  public async handleMessage(msg: any): Promise<void> {
    const { handlePanelMessage } = require('../../../core/routing/chatPanelMessageRouter.js');
