...
export function renderHeaderRightInner(header?: ChatHeaderInfo): string {
  return `${header && header.hasProjectOpen ? `
      ...
      <button class="header-btn header-btn--phone" data-cmd="redivivus.addToPhone" title="Add to Phone — turn this project into an installable app (get a QR + link to scan and install on any device)">&#x1F4F1; Add to Phone</button>
      ` : ''}
