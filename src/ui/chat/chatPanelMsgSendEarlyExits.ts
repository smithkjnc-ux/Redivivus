...
export async function handleUrlRead(
  ...
  const { extractUrl, readUrl } = await import('../../services/webSearchService.js');
...
export async function handleWebSearch(
  ...
  const { extractUrl, detectSearchIntent, searchWeb } = await import('../../services/webSearchService.js');
  const pastedUrl = extractUrl(userText);
  const searchQuery = detectSearchIntent(userText);
  if (!searchQuery || pastedUrl) { return false; }
...
export async function handleReadResult(
  ...
  const { readUrl } = await import('../../services/webSearchService.js');
