  ...
  return `
    ...
    function showNewProjectPanel(suggestedParent, prefillTask, compact, vaultOnly, prefillAnswers) {
      ...
      if (compact && prefillTask) {
        ...
        const cn=document.createElement('button'); cn.textContent='Cancel'; cn.style.cssText='padding:8px 18px;border:1px solid #2d3a55;border-radius:8px;background:transparent;color:#8899bb;cursor:pointer;font-size:13px;'; cn.addEventListener('click',()=>ov.remove()); btns.appendChild(cn);
        ...
        if (vaultOnly) { const proj=document.createElement('button'); proj.textContent='Make it a Project Instead'; proj.style.cssText='padding:8px 18px;border:1px solid #2d3a55;border-radius:8px;background:transparent;color:#e8edf8;cursor:pointer;font-size:13px;'; proj.addEventListener('click',()=>{ov.remove();vscode.postMessage({type:'show-panel',panelType:'create-folder',prefillName:ta.value.trim().replace(/[^a-zA-Z0-9\\s]/g,'').trim().split(/\\s+/).slice(0,4).join('-').toLowerCase(),pendingTask:ta.value.trim()});});  btns.appendChild(proj); }
      ...
      function renderStep() {
        ...
        if (step < questions.length) {
          ...
          const cn=document.createElement('button'); cn.textContent='Cancel'; cn.style.cssText='padding:8px 18px;border:1px solid #2d3a55;border-radius:8px;background:transparent;color:#8899bb;cursor:pointer;font-size:13px;'; cn.addEventListener('click',()=>overlay.remove()); btns.appendChild(cn);
