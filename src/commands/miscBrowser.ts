// [SCOPE] CHASSIS Misc commands — browser, project list
...
export function registerBrowserCommands(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('chassis.listProjects', async () => {
          ...
          try {
            ...
            for (const entry of entries) {
              if (entry.isDirectory()) {
                ...
                if (fs.existsSync(path.join(projectPath, '.chassis'))) { projects.push({ name: entry.name, path: projectPath }); }
              }
            }
      ...
      if (projects.length === 0) { vscode.window.showInformationMessage('No CHASSIS projects found. Create one with "Start New Project Setup"'); return; }
      const items = projects.map(p => ({ label: p.name, description: p.path, detail: p.path }));
      const selected = await vscode.window.showQuickPick(items, { placeHolder: 'Select a CHASSIS project to open' });
