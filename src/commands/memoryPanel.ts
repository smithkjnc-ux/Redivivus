  ...
  private constructor(panel: vscode.WebviewPanel) {
    ...
    this._panel.webview.onDidReceiveMessage(async (msg) => {
      if (msg.type === 'delete-explicit') {
        const { removeExplicit } = await import('../services/userMemoryService.js');
        removeExplicit(msg.index);
        this._render();
      } else if (msg.type === 'add-explicit') {
        const { rememberExplicit } = await import('../services/userMemoryService.js');
        ...
        try {
          const root = msg.root;
          if (!root) { return; }
          const { readKnowledge, writeKnowledge } = await import('../services/learnedMemoryServiceIO.js');
