...
// temperature: role-based temp passed from orchestrator (supervisor ~0.4, worker ~0.2, guardian ~0.0).
//              Provider floor clamps are applied inside each executor — never block on this.
export async function callProvider(
  ...
  switch (ai) {
    case 'gemini':
      return executeGemini(text, fetchWithTimeout, tier, imageBase64, imageType, systemMessage, temperature);
    case 'claude':
      return executeClaude(text, fetchWithTimeout, tier, imageBase64, imageType, systemMessage, temperature);
    case 'openai':
      return executeOpenAI(text, fetchWithTimeout, systemMessage, tier, imageBase64, imageType, temperature);
    case 'groq':
    case 'xai':
    case 'kimi':
    case 'deepseek': {
