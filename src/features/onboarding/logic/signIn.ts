...
import { setAccountToken, getAccountToken, clearAccountToken } from '../../../features/api/data/apiClient.js';
    ...
    vscode.commands.registerCommand('redivivus.signIn', async () => {
      ...
      if (current) {
        ...
        if (choice === 'Sign out') {
          ...
          try { const { ChatPanel } = await import('../../chat/ui/chatPanel.js'); (ChatPanel as any).currentPanel?.refresh(); } catch {}
          return;
        }
      }
