...
export function generateProjectMap(result: AnalysisResult, root: string): string {
  ...
  md += `*Generated by Redivivus Analyzer*\n\n---\n\n`;
  md += `## Overview\n\n`;
  md += `| Metric | Count |\n|--------|-------|\n`;
  ...
  if (result.todoItems.length > 0) {
    ...
    md += `Convert these to Redivivus annotations (\`// [TODO]\`, \`// [DONE]\`, etc.):\n\n`;
  ...
  if (result.missingScopeFiles && result.missingScopeFiles.length > 0) {
    ...
    md += `These code files lack the mandatory Redivivus scope tag at their very first line:\n\n`;
...
export function generateRecommendations(result: AnalysisResult): string {
  let md = `# Redivivus Recommendations\n\n`;
  ...
  if (result.todoItems.length > 0) {
    md += `### ${priority++}. Convert TODOs to Redivivus Annotations\n\n`;
    md += `Found ${result.todoItems.length} existing TODO/FIXME markers. Convert them:\n\n`;
    md += `| Old Style | Redivivus Style |\n|-----------|---------------|\n`;
  ...
  md += `Redivivus will flag violations automatically in future versions.\n\n`;
  md += `---\n\n`;
  md += `*These recommendations are structural. Deep code analysis requires Redivivus Phase 2 (AI routing).*\n`;
  return md;
}

// [SCOPE] Analyze a single file and return a markdown report of its current Redivivus state
export async function analyzeCurrentFile(filePath: string, fileContent: string): Promise<string> {
  ...
  report += '*Generated by Redivivus Analyzer*\n\n---\n\n';
  ...
  report += '- **Redivivus tags:** ' + (found.length > 0 ? found.join(', ') : 'None') + '\n';
  ...
  report += '---\n\n## Planned Redivivus Changes\n\n';
  const planned: string[] = [];
  if (!found.some(f => f.startsWith('[SCOPE]'))) { planned.push('Add ' + commentChar + ' [SCOPE] at top'); }
  if (oldTodos.length > 0) { planned.push('Convert ' + oldTodos.length + ' TODO/FIXME items to Redivivus format'); }
  ...
  if (planned.length === 0) { planned.push('File already meets Redivivus standards'); }
