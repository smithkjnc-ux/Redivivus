...
export async function isModificationRequest(taskLow: string, routing: RoutingService, usageTracker?: UsageTracker): Promise<boolean> {
  ...
  try {
    ...
    if (usageTracker && res.inputTokens) {
      usageTracker.recordUsage(Math.ceil((res.inputTokens + (res.outputTokens || 0)) / 4), 0, (res.model && res.model !== 'none') ? res.model : routing.getAvailableAI().ai, res.inputTokens, res.outputTokens, 'qa');
    }
