...
import { _scanChassisProjects } from '../../ui/chassisProjectScanner';
...
export async function handleKeywordShortcuts(
  ...
  const { conversation, refresh, panel, chassis } = deps;
  ...
  if (/how'?s\s+my\s+setup|setup\s+progress|what'?s\s+left|what\s+to\s+do\s+next/i.test(lowerText)) {
    ...
    if (_spRoot && chassis) {
      ...
      const svc = new SetupProgressService(chassis, _spRoot);
