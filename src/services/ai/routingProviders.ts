  ...
  if (ai === 'gemini') {
    ...
    try {
      ...
      if (finishReason === 'MAX_TOKENS') {
        return { text: responseText, model, success: false, error: `Gemini response was truncated (hit output token limit). Response may be incomplete.` };
      }
      return { text: responseText, model, success: true, usingFallback: undefined };
