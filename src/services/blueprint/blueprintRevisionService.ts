    ...
    const scanDir = (dir: string, depth = 0) => {
      ...
      try {
        for (const name of fs.readdirSync(dir)) {
          if (['.chassis', 'node_modules', '.git', '.github'].includes(name)) { continue; }
...
---
*Generated by CHASSIS v${bp.version || '0.3.6'}*
`;
...
export async function tryBlueprintRevision(root: string, chassis: any, routing: any): Promise<void> {
