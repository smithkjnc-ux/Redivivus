// [SCOPE] Rules Generator — creates AI editor instruction files so all tools respect CHASSIS annotations
...
export class RulesService {
  ...
  generateAll(root: string, projectName: string): string[] {
    const rules = this.buildRules(projectName);
    const created: string[] = [];

    const targets: { file: string; content: string }[] = [
      { file: 'CLAUDE.md', content: this.wrapForClaude(rules, projectName) },
      { file: 'GEMINI.md', content: this.wrapForGemini(rules, projectName) },
  ...
  private buildRules(projectName: string): string {
...
`;
  ...
  private wrapForClaude(rules: string, projectName: string): string {
...
`;
  ...
  private wrapForGemini(rules: string, projectName: string): string {
