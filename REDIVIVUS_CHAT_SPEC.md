# Redivivus Chat & Developer Experience Spec
> **Rule:** This is the design spec for Redivivus's integrated chat and developer-first features. Read before implementing. Update after changes.

---

## Vision
Redivivus turns plain VS Code into a complete AI coding environment that costs pennies, remembers everything, and gets smarter with every project. No subscription. No lock-in. Built from a coder's point of view.

**One sentence:** "The vibe editor that remembers."

---

## 1. Redivivus Chat Panel

### Location
- VS Code sidebar panel (same position as Windsurf's Cascade)
- Own activitybar icon (the Redivivus frame icon)
- `Ctrl+L` opens/focuses the chat (matches Windsurf muscle memory)
- Always visible when Redivivus is active — no hunting for it
...
- **Vault-first search** — before calling AI, Redivivus checks the vault for matching code. Shows what it found vs what it needs to generate
...
- **Conversation history** — persists in `.redivivus/chat/` as JSON. Survives editor restarts
...
- **Multi-file awareness** — "refactor the auth flow" can touch multiple files. Redivivus shows all changes as a batch
...
1. **Vault search first** — they generate from scratch every time. Redivivus reuses proven code
2. **Cost transparency** — they hide token usage. Redivivus shows every penny
3. **Blueprint context** — they don't know what your project is FOR. Redivivus does
4. **Session memory** — they forget between conversations. Redivivus tracks progress
5. **Undo per-change** — they have global undo. Redivivus has per-AI-change undo
6. **Gets cheaper over time** — they cost the same forever. Redivivus costs less as vault grows
...
### Concept
A non-coder should NEVER have to open a terminal to save their work. Redivivus handles git automatically like a save system in a video game.

### Auto-Save Points
- **After every successful build** — Redivivus auto-commits with a generated message
...
```
[Redivivus] {action}: {description}
```
Examples:
```
[Redivivus] build: countdown timer with start/pause/reset
[Redivivus] fix: auth token refresh not firing on app resume
[Redivivus] session-end: Fixed avatar display, next: wire dashboard
[Redivivus] save-point: before refactoring auth flow
```

### GitHub Integration (Built-in)
- **First time setup:** Redivivus asks "Want to back up your code to GitHub?" → walks through token setup → creates repo → pushes
- **Auto-push** — every commit auto-pushes to GitHub (toggleable)
- **No terminal needed** — everything through the Redivivus UI
- **Branch management** — Redivivus creates branches for risky changes: "This is a big refactor. Working on a safety branch." Auto-merges when user approves
...
│ │   └─ goal: finish applying redivivus │
...
- `.redivivus/work_log.md` — session starts, ends, analyses, reviews, retrofits
- `.redivivus/sessions/*.json` — detailed session data
- `.redivivus/chat/` — conversation history (new)
- Git log — commits and save points
- Vault scan history — what was extracted and when

### What Makes This Different
No editor shows you the STORY of your project. They show files. Redivivus shows the journey — when you started, what you tried, what failed, what worked, how the code evolved. For a solo developer, this is your memory. For a team, this is your documentation.
...
- "Redivivus: Review this file" → AI review
- "Redivivus: Clean up this file" → add annotations
- "Redivivus: Save to vault" → extract reusable blocks
- "Redivivus: Check file health" → guardian scan

### Right-click selected code in editor
- "Redivivus: Explain this" → AI explains in chat
- "Redivivus: Improve this" → AI suggests improvements
- "Redivivus: Save to vault" → save selection as vault item
- "Redivivus: Find similar in vault" → search vault for matching code

### Right-click a folder
- "Redivivus: Scan folder" → analyze all files in folder
- "Redivivus: Scan for vault" → extract vault items from folder
...
| Shortcut | Action |
|----------|--------|
| `Ctrl+L` | Open/focus Redivivus chat |
| `Ctrl+Shift+C` | Open Redivivus dashboard |
...
```
🏗️ Redivivus: Draft │ 🟢 Session: 47m │ ⚡ 1,247 tokens ($0.002) │ 💾 Auto-saved 3m ago
```
...
When Redivivus is installed in plain VS Code for the first time:

1. **Welcome panel** — "Hey! I'm Redivivus. I help you build software by remembering what works."
...
## 8. What Redivivus Does NOT Do (Stay in Lane)

- **No inline autocomplete** — that's Copilot's territory. Redivivus is conversational, not predictive
- **No file editing UI** — VS Code's editor is perfect. Don't rebuild it
- **No terminal replacement** — VS Code's terminal works. Redivivus just doesn't REQUIRE it
...
"Redivivus turns free VS Code into an AI coding studio for under $5 a year. And it gets smarter every time you use it."

**For the developer who's tired of AI amnesia:**
"Your AI forgets everything between sessions. Redivivus doesn't. Blueprint, vault, work log, timeline — your project's entire history, always available."

**For the solo builder:**
"Stop re-explaining your project to AI every session. Redivivus remembers the blueprint, knows the rules, and reuses what worked before."
