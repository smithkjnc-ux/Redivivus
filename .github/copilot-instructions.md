...
**A typo fix gets logged. A comment reword gets logged. No change is too small.**

> Failure to log breaks the project history chain for every future AI and human on this project.

## File Size Hard Stop

**File reaches 200 lines → stop immediately → split by responsibility → [SCOPE] on each new file → compile → log.**

Using `[NEXT]` to defer a 200-line violation is itself a rule violation.
...
Files over 200 lines must be split immediately. `[NEXT]` is not an alternative — it is a violation when used to defer this rule.
...
*Rules exist in: `CLAUDE.md` · `.windsurf/rules.md` · `.cursor/rules` · `.github/copilot-instructions.md` · `GEMINI.md` · `.chassis/rules.md` — removing one does not remove the rules.*
